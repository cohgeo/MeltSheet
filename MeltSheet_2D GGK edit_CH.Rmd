...
# Define material properties, initialize arrays.
  ...
  Cp.nolatent <- 1000  # heat capacity with no latent heat, [J/(kg*K)]
                       # Suggested value: 1000 J/(kg/K)
  k <- matrix(data = 2.5,    # thermal conductivity, [W/(m*K)]
              nrow = z.num,  # Suggested value: 2.5 W/(m*K)
              ncol = x.num)  
  A <- matrix(data = 0,      # heat production, [W/(m^3)]
              nrow = z.num,  # Suggested value: 1e-6 W/(m^3)
              ncol = x.num)  
  rho <- matrix(data = 2700,   # density, [kg/(m^3)]
                nrow = z.num,  # Suggested value: 2700 kg/(m^3)
                ncol = x.num)
  Cp.initial <- matrix(data = Cp.nolatent,   # heat capacity, [J/(kg*K)]
                       nrow = z.num,  
                       ncol = x.num)
  Cp.prime <- Cp.initial + (L / (T.liquidus - T.solidus))
    # heat capacity, [J/(kg*K)]
    # Suggested value: 3339 J/(kg*K)
      ...
      # next iteration of the model
        for (i in 1:x.num) {
          for (j in 1:z.num) {
            ...
            Cp[[t + 1]][i, j] <- ifelse(T.n[[t + 1]][i, j] < T.solidus && T.n[[t + 1]][i, j] > T.liquidus, Cp.initial[i, j], Cp.prime[i, j])
            
            
            # Set value of Cp.
            
            # if (T.n[[t + 1]][i, j] > T.solidus & T.n[[t + 1]][i, j] < T.liquidus) {
            #   Cp[[t + 1]][i, j] <- Cp.initial[i, j]   # heat capacity, [J/(kg*K)]
            # } else {
            #   Cp[[t + 1]][i, j] <- Cp.prime[i, j]
            # }
...
for (i in 1:200) {
          for (j in 1:200) {
            # Pull out comparison temperature.
            


            if (T.n[[2]][i, j] > T.solidus & T.n[[2]][i, j] < T.liquidus) {
              Cp.test[[1]][i, j] <- 55 #Cp.initial[i, j]   # heat capacity, [J/(kg*K)]
            } else {
              print("bad")
              #Cp.test[[1]][i, j] <- 33 #Cp.prime[i, j]
            }
          # Cp[[t + 1]][i, j] <- 2  
...
if (T.n[[2]][3, 5] > T.solidus & T.n[[2]][3, 5] < T.liquidus) {
              print("good")#Cp.test[[1]][i, j] <- 55 #Cp.initial[i, j]   # heat capacity, [J/(kg*K)]
            } else {
              print("bad")
              #Cp.test[[1]][i, j] <- 33 #Cp.prime[i, j]
            }
...
T.n.test <- T.n[[2]]
for (i in 1:3) {
  test.CP[i] <- ifelse(T.n.test[i, 5] > T.solidus & T.n.test[i, 5] < T.liquidus, "bad", "good")
