...
# caused by an impact. 
# In this case we model the cooling of the Morokweng melt sheet with a minimum
# Cp.prime value.
...
# Save model results to computer. Change path for your working directory.
  saveRDS(T.n, file = "Morokweng Results/MeltSheet Results_2020.12.07/T.n_Cp.prime.min.RData")
...
# Zoom in on samples to see when each sample passes through the solidus.
  ...
  T.1D.lim2 <- list()
  lim2.iterations <- c(114, 116, 118,  # M3-1
                       188, 189, 190,  # M3-2
                       239, 240, 241,  # M3-3
                       261, 262, 263,  # M3-4
                       276, 277, 278)  # M3-6
  ...
  # Make plot.
  plot.new()  
  plot(range(750, 800), range(0.4, 1.2),
       type = "n",
       xlab = "Temperature (°C)",
       ylab = "Depth (km)",
       ylim = c(1.2, 0.4),
